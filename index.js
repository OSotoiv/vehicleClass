...
class Motrocycle extends Vehicle {
    constructor(make, model, year) {
        super(make, model, year)
        this.numWheels = 2;
    }
...
class Garage {
    constructor(cap) {
        if (!cap || !Number.isFinite(cap)) { throw new Error('A garage needs to hold at least one vehicle') };
        this.vehicles = [];
        this.capacity = cap
    }
    add(vehicle) {
        if (vehicle.__proto__ !== Vehicle.prototype) { return "Only vehicles are allowed in here!" };
